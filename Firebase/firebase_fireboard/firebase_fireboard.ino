...
void loop() {
 ...
 //Humidity
   json.clear().add("Humidity", h);
  json1.clear().add("Temperature", t);
  json2.clear().add("time", NowString());
  a.clear().add("Humidity", h).add("Temperature", t).add("time", NowString()) ;
    ...
    if (Firebase.pushJSON(fbdo, path , a))
    {
      //Firebase.pushJSON(fbdo, path + "/Data:", json1);
     // Firebase.pushJSON(fbdo, path + "/Data:", json2);
